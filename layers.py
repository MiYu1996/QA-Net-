...
class Embedding(nn.Module):
    ...
    def __init__(self, word_vectors, hidden_size, drop_prob, char_vectors):
        ...
        self.char_embed_size = char_vectors.size()[1]
        self.word_embed_size = word_vectors.size()[1]
        self.max_word_len = 16   # args.py (char_limit)
        self.kernel_size = 5

        self.cnn = CNN(self.char_embed_size, self.max_word_len, self.word_embed_size, self.kernel_size)

        self.proj = nn.Linear(2 * self.word_embed_size, self.hidden_size, bias=False)
    ...
    def forward(self, word_x, char_x):
        ...
        w_emb = self.word_embed(word_x)     # torch.Size([64, 338, 300])
...
class SelfAttention(nn.Module):
    ...
    Directly match question-aware passage representation (in our case, the BIDAF attention layer) against itself
    ...
    def __init__(self, input_size, hidden_size, dropout):
        ...
        self.WP = nn.Linear(2*self.hidden_size, 2*self.hidden_size, bias=False)
        self.linear2 = nn.Linear(in_features = 6*self.hidden_size, out_features = 2*self.hidden_size, bias=True)
