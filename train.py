...
# Version: V6
# Model: QANet
# Optimizer: Adam, QANet hyperparameters
...
def main(args):
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-3, betas=(0.8, 0.999),
                            eps=1e-7, weight_decay=3e-7, amsgrad=False)
    scheduler = sched.LambdaLR(optimizer, lambda s: 1.)  # Constant LR
        ...
        with torch.enable_grad(), \
            ...
            for cw_idxs, cc_idxs, qw_idxs, qc_idxs, y1, y2, ids in train_loader:
                ...
                if steps_till_eval <= 0:
                    ...
                    ### Start our code:
                    # results2, pred_dict2 = evaluate(model, train_loader, device,
                    ...
                    #
                    # # Log to console
                    # results_str2 = ', '.join('{}: {:05.2f}'.format(k, v)
                    #                         for k, v in results2.items())
                    # log.info('Train {}'.format(results_str2))
                    #
                    # # Log to TensorBoard
                    # log.info('Visualizing in TensorBoard...')
                    # for k, v in results.items():
                    #     tbx.add_scalar('train/{}'.format(k), v, step)
                    # util.visualize(tbx,
...
### start our code:
def LR_decay(optimizer, epoch, lr = 0.5, decay_epoch = 1):
    """Function to decay the initial learning rate by 0.97 every decay_epoch epochs.
    """
    lr = lr * (0.97**( (epoch-1) // decay_epoch))
    if lr < 0.3:  # lower bound of learning rate
        lr = 0.3
...
def LR_warmup(optimizer, step, lr = 0.):
    """Function to do learning rate warmup for first 1000 batches
    """
    step = step/32
    if step < 10:
        lr = 0.0001
    elif step < 1000:
        lr = (1e-3 / 3) * math.log10(step+1)
    else:   # lower bound of learning rate
        lr = 1e-3
